...
const Player = (() => {
  ...
  function togglePlay() {
    ...
    if (hasRealAudio()) {
      ...
      if (state.isPlaying) {
        if (state.device !== "browser") API.stopServer();
        state.isPlaying = false; stopVirtual();
      } else {
        if (state.device !== "browser") API.playServer(state.current);
        state.isPlaying = true; startVirtual();
      }
      setPlayIcon();
    }
  }
  ...
  function startVirtual() {
    ...
    virtualTimer = setInterval(() => {
      ...
      if (state.time >= state.duration) { state.time = state.duration; onEnded(); return; }
      el.tCur.textContent = fmt(state.time);
      el.tTot.textContent = fmt(state.duration);
      updateWaveProgress();
